                ...
                if type(groups) == bool:
                    ...
                    for group in groups:
                        ...
                        for contact in group.get("contacts", []):
                            ...
                            if existing_user is None and user_id is not None:
                            ...
                            if group_user is None and existing_user is not None:
                                new_group_user = GroupUser(group_id=group_id, user_id=user_id)
                                session.add(new_group_user)
                                await session.commit()

        except KeyError as e:
            logger.error(f'Error! iterator: {iterator} | error_code: {e} | error_msg: {row_json}')
            err.append(iterator)
